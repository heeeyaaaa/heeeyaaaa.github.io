...
Also you will want to delete all alerts that get written directly to your host eg. from audit.d.

sudo nano /etc/systemd/system/wazuh-log-cleanup.service

```bash
[Unit]
Description=Clean old Wazuh alert logs older than 30 days

[Service]
Type=oneshot
ExecStart=/usr/bin/find /var/lib/docker/volumes/single-node_wazuh_logs/_data/alerts \
  -mindepth 2 -maxdepth 2 -type d \
  -mtime +30 -exec rm -rf {} +
```

sudo nano /etc/systemd/system/wazuh-log-cleanup.timer

```bash
[Unit]
Description=Daily Wazuh log cleanup
...
This runs daily and nukes any top-level subdirectory in alerts older than 30 days,  month dirs like 2026, Feb, Mar.
